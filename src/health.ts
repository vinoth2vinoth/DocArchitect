...
import { DocArchitect, SourceFileBlock } from './DocArchitect.js';
...
export interface Recommendation {
  ...
  category: 'coverage' | 'freshness' | 'links' | 'efficiency';
  message: string;
  action?: string;
}
...
export class DocumentationHealth {
  ...
  constructor(docsRoot: string, sourceRoot: string, cacheDir: string, mappings: Record<string, string>) {
  ...
  private async analyzeCoverage(): Promise<HealthMetrics['coverage']> {
  ...
  private async analyzeFreshness(): Promise<HealthMetrics['freshness']> {
  ...
  private async analyzeLinkHealth(): Promise<HealthMetrics['linkHealth']> {
    let totalLinks = 0;
    let brokenLinks = 0;
    ...
    for (const docFile of Object.values(this.mappings)) {
      ...
      while ((match = linkPattern.exec(content)) !== null) {
        ...
        if (!target || /^(https?:|mailto:|#)/i.test(target) || target.startsWith('data:')) continue;
  ...
  private async analyzeTokenEfficiency(): Promise<HealthMetrics['tokenEfficiency']> {
  ...
  private calculateOverallScore(
    coverage: HealthMetrics['coverage'],
    freshness: HealthMetrics['freshness'],
    linkHealth: HealthMetrics['linkHealth'],
    tokenEfficiency: HealthMetrics['tokenEfficiency']
  ): number {
  ...
  printReport(metrics: HealthMetrics): void {
    console.log('\n' + '='.repeat(60));
