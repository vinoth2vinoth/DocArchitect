...
export interface DocConfig {
  ...
  cacheDir?: string;
  smartBudget?: boolean;
  budgetStrategy?: 'balanced' | 'coverage' | 'depth';
}
...
export class DocArchitect {
  ...
  private fileImportanceHistory: Map<string, number> = new Map();
  ...
  private loadImportanceHistory(): void {
    try {
      const historyPath = path.join(this.config.cacheDir, 'importance-history.json');
      if (fs.existsSync(historyPath)) {
        const data = JSON.parse(fs.readFileSync(historyPath, 'utf-8'));
        this.fileImportanceHistory = new Map(Object.entries(data).map(([k, v]) => [k, v as number]));
  ...
  private saveImportanceHistory(): void {
    try {
      fs.ensureDirSync(this.config.cacheDir);
      const historyPath = path.join(this.config.cacheDir, 'importance-history.json');
  ...
  private extractTypeScriptSignatures(content: string, filePath: string): Map<string, string> {
...
interface SmartBudgetOptions {
  smartBudget?: boolean;
  budgetStrategy?: 'balanced' | 'coverage' | 'depth';
  maxTokens?: number;
  fileImportanceHistory?: Map<string, number>;
}

export function buildCodeContext(
  files: SourceFileBlock[], 
  maxChars: number,
  options?: SmartBudgetOptions
): string {
  const {
    smartBudget = false,
    budgetStrategy = 'balanced',
  ...
  let sortedFiles: (SourceFileBlock & { importanceScore?: number })[] = [...files];
...
export function calculateFileImportance(file: SourceFileBlock): number {
  let score = 5;
...
export function hasChangesInCurrentDoc(
  currentDoc: string,
  fileSignatures: any[],
  cachedEntry: any
): boolean {
